...
### 【6.5】
1. 新增【轨道模拟器】，可模拟合成屋制卡强卡（有隐藏彩蛋哦）。
2. 轨道强化系统新增VIP16。
3. 公会任务取消任务卡必须不转的要求。
4. 修复游戏更新后无法进入巅峰对决的问题。
5. 修复部分用户无法使用兑换功能的问题。
6. 新增赞赏致谢名单。
#### 【6.5.1】
1. 模拟器优化：星级上限提升，新增10星香料和蛇草，新增4种卡片（含中差卡），初始金币和点券增加10倍，修复星级图标、金币显示bug。
2. 【检查任务】可检查的防御卡数量增加至50张。
3. 修复网络卡顿时误报【好卡配方耗尽】的bug。
4. 赞赏致谢名单更新。
...
6. 删除魔塔轨道里的火兔和忍鸡。
7. 修复选服点到充值活动的问题。
8. 修复音乐节地图变化导致麦芽糖补阵失效的问题。
9. 修复win7无法识别魔塔次数的问题。
10. bug修复：送花无法执行；电脑连续运行50天导致计时器溢出；温馨礼包openid太长导致执行器闪退；控制类任务多显示一个灰色横杠。
11. 新增“赞赏作者”功能，您的赞赏将助力轨道更好的发展。

### 【6.3】
1. 更新本期大赛。
2. “跨服”类型新增沙漠跨服。
3. 任务列表用红绿灯指示任务完成情况。
4. 任务列表允许从中间启动。双击某个任务ID，即可从此处开始执行。
5. 任务列表新增暂停功能（以局为单位，第5局中途暂停则继续执行时从第5局开始）。
6. 任务列表中允许存在多个自动爬塔任务。
7. 登录失败触发刷新时只刷新登录失败的账号。
8. 自动通过动态验证（需下载动态验证DLC）。
9. bug修复：“控制-刷新”任务会刷新没有勾选的账号；强化13星以上卡片无法识别结果；保鲜袋干扰香料识别。

### 【6.2】
1. 自动爬塔加强：不再只能使用同一个轨道爬塔，现在可以识别层数自动匹配轨道。（使用手册3.2.13）
2. 新增魔塔1-100层预制轨道。
3. 魔塔次数不足时自动跳过剩余局数，而不是反复刷新。
4. 新增“强化模拟”功能：模拟真实强化过程，计算当前材料能强化出什么卡片。（使用手册3.3.3）
5. 人物等级1级也能使用高级任务，轨道会根据等级选择跳转的岛屿。
6. 允许在列表中启动其他高级任务。（使用手册3.2.14）
7. 对话框字体稍微增大。
8. bug修复：单人检查任务出错；定时退出失效；使用<>指令替换的卡第2局以后不放卡。
#### 【6.2.1】
1. bug修复：宠塔、密室误报“魔塔次数不足”；强化数据上传出错。
